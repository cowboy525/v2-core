...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (cic.newlyDeployed) {
    ...
    await execute("RadiantOFT", { from: dao, log: true }, "transfer", cic.address, config.SUPPLY_CIC_RESERVE);
    await execute("ChefIncentivesController", { from: deployer, log: true }, "registerRewardDeposit", config.SUPPLY_CIC_RESERVE);
    await execute("EligibilityDataProvider", { from: deployer, log: true }, "setChefIncentivesController", cic.address);
  }
};
export default func;
func.tags = ['core'];
