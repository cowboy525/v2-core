...
import fs from "fs";
import { DeployConfig, DeployConfigOverride, DeployData, FixtureDeploy } from "../../scripts/deploy/types";
import { AToken, BountyManager, ChefIncentivesController, LendingPool, Leverager, MFDPlus, MiddleFeeDistribution, MockToken, RadiantOFT, UniV2TwapOracle, VariableDebtToken } from "../../typechain-types";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";
import { LockZap } from "../../typechain-types/contracts/zap/LockZap";
import { WETH } from "../../typechain-types/contracts/misc/WETH";
import { TestnetLockZap } from "../../typechain-types/contracts/test/TestLockZap.sol";
import { MultiFeeDistribution } from "../../typechain-types/contracts/staking";
import { MINUTE } from "../../config/constants";
import { WBNB, WETHGateway } from "../../typechain-types/contracts/misc";
...
export const approxEqual = (val1: BigNumber, val2: BigNumber, threshold: number = .1, decimals: string = "1000000000000000000") => {
...
export async function testFullDeploy(configOverrides: DeployConfigOverride = {}): Promise<any> {
  // const [deployUser] = await ethers.getSigners();
  // const { chainId } = await ethers.provider.getNetwork();

  // const deployer = new Deployer(chainId, deployUser, configOverrides);
  // const { deployData, config } = await deployer.deploy(ethers.constants.AddressZero, {
  //   logs: false,
  // });
...
export const depositRadiantToCic = async (rdnt: RadiantOFT, user: SignerWithAddress, cic: ChefIncentivesController, amount: BigNumberish) => {
...
export const getTotalPendingRewards = async (address: string, chef: ChefIncentivesController) => {
  const pendingRewards = await chef.allPendingRewards(
    address,
  );
...
export async function setAllocPoints(chef: ChefIncentivesController, data: any) {
  ...
  const receipt = await chef.batchUpdateAllocPoint(
    tokens,
    allocPoints
  );
  await receipt.wait();
...
export const sellRdnt = async (amt: string, user: SignerWithAddress, radiant: RadiantOFT, lockZap: TestnetLockZap, priceProvider: PriceProvider, stakingToken: string) => {
  ...
  await radiant.connect(user).approve(
    lockZap.address,
...
export const zapIntoEligibility = async (user: SignerWithAddress, deployData: DeployData, amountEth: string = "10", lockLengthIndex: number = 0, borrow = false) => {
  ...
  await delay2();
  if (borrow) {
    await (await lockZap.connect(user).zap(
      borrow,
      amountEth,
      0,
      lockLengthIndex,
    ...
    await lockZap.connect(user).zap(
      borrow,
      0,
      0,
      lockLengthIndex,
...
export const depositAndBorrowAll = async (user: SignerWithAddress, amounts, deployData, delay: boolean = false) => {
  let keys = ['rUSDT', 'rUSDC', 'rDAI', 'rWETH', 'rWBTC', 'rGLP', 'rFRAX'];
  // let keys = ['rWBTC'];
  // let keys = ['rWETH'];
...
export const depositAndBorrowAllBsc = async (user: SignerWithAddress, amounts, deployData, delay: boolean = false) => {
  let keys = ['rUSDT', 'rBUSD', 'rDAI', 'rWETH', 'rWBTC', 'rWBNB'];
...
export const depAll = async (user: SignerWithAddress, amounts, deployData, delay: boolean = false, all = false) => {
  let keys;
  if (all) {
    keys = ['rUSDT', 'rUSDC', 'rDAI', 'rWETH', 'rWBTC', 'rFRAX', 'rGLP'];
  } else {
    keys = ['rUSDC'];
    // keys = ['rWETH'];
  }
...
export const borAll = async (user: SignerWithAddress, amounts, deployData, delay: boolean = false, all = false) => {
  let keys;
  if (all) {
    keys = ['rUSDT', 'rUSDC', 'rDAI', 'rWETH', 'rWBTC', 'rGLP', 'rFRAX'];
  } else {
    keys = ['rUSDC'];
  }
...
// export function customFixture(configOverrides: DeployConfigOverride = {}) {
//   const inner = () => {
...
// export async function deployAndSetup(configOverrides: DeployConfigOverride = {}): Promise<FixtureDeploy> {
//   let deployer: SignerWithAddress;
//   let dao: SignerWithAddress;
//   let treasury: SignerWithAddress;
//   let team: SignerWithAddress;
//   let eco: SignerWithAddress;
...
//   let LOCK_DURATION;
//   let REWARDS_DURATION;

//   let deployData: DeployData;
//   let deployConfig: DeployConfig;

//   [deployer, dao, treasury, team, eco, user1, user2, user3, user4] = await ethers.getSigners();

//   configOverrides.DAO = deployer.address;
//   configOverrides.TREASURY = treasury.address;
//   configOverrides.TEAM_RECEIVER = team.address;
//   configOverrides.ECOSYSTEM_RECEIVER = eco.address;

//   const deploy = await setupTest();
...
//   REWARDS_DURATION = (await multiFeeDistribution.REWARDS_DURATION()).toNumber();
//   LOCK_DURATION = (await multiFeeDistribution.DEFAULT_LOCK_DURATION()).toNumber();

//   rdntToken = await ethers.getContractAt("RadiantOFT", deployData.rdntToken);

//   lpFeeDistribution = <MFDPlus>(
//     await ethers.getContractAt("MFDPlus", deployData.lpFeeDistribution)
//   );
//   lockZap = <TestnetLockZap>(
//     await ethers.getContractAt("TestnetLockZap", deployData.lockZap)
//   );

//   leverager = <Leverager>(
//     await ethers.getContractAt("Leverager", deployData.leverager)
//   );
...
//       deployData.eligibilityDataProvider
//     )
//   );

//   uniV2TwapOracle = <UniV2TwapOracle>(
//     await ethers.getContractAt("UniV2TwapOracle", deployData.uniV2TwapOracle)
//   );

//   const wethGateway = <WETHGateway>(
//     await ethers.getContractAt("WETHGateway", deployData.wethGateway)
//   );

//   priceProvider = <PriceProvider>(
//     await ethers.getContractAt("PriceProvider", deployData.priceProvider)
//   );

//   bountyManager = <BountyManager>(
//     await ethers.getContractAt("BountyManager", deployData.bountyManager)
//   );

//   const usdcAddress = deployConfig.TOKENS_CONFIG[0][1].assetAddress;
//   const wethAddress = deployConfig.TOKENS_CONFIG[4][1].assetAddress;

//   const usdc = <MockToken>await ethers.getContractAt("MockToken", usdcAddress);
//   const weth = <WETH>await ethers.getContractAt("WETH", deployData.baseAssetWrappedAddress);
...
//     deployData,
//     deployConfig,
//     deployer, dao, treasury, team, eco, user1, user2, user3, user4,
//     lendingPool,
//     chefIncentivesController,
//     multiFeeDistribution,
//     REWARDS_DURATION,
