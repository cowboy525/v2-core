...
describe("Stargate Borrow", () => {
  ...
  before(async () => {
    ...
    variableDebtUSDC = <VariableDebtToken>(
      await ethers.getContractAt(
        "VariableDebtToken",
        deployData.allTokens.vdUSDC
      )
    );
    variableDebtWETH = <VariableDebtToken>(
      await ethers.getContractAt(
        "VariableDebtToken",
        deployData.allTokens.vdWETH
      )
    );
  ...
  it("setDAOTreasury", async () => {
    await expect(
      stargateBorrow.connect(user2).setDAOTreasury(treasury.address)
  ...
  it("setXChainBorrowFeePercent", async () => {
    await expect(
      stargateBorrow
        .connect(user2)
        .setXChainBorrowFeePercent(deployConfig.FEE_XCHAIN_BORROW)
    ).to.be.revertedWith("Ownable: caller is not the owner");
    await stargateBorrow.setXChainBorrowFeePercent(
      deployConfig.FEE_XCHAIN_BORROW
    );
  });
  ...
  it("Check X Chain Borrow Fee", async () => {
    ...
    await USDC.connect(user2).approve(
      stargateBorrow.address,
      ethers.constants.MaxUint256
    );

    await variableDebtUSDC
      .connect(user2)
      .approveDelegation(stargateBorrow.address, ethers.constants.MaxUint256);
    ...
    await stargateBorrow
      .connect(user2)
      .borrow(usdcAddress, borrowAmt, 2, 10143);
  ...
  it("borrow eth", async () => {
    ...
    await wrappedEth
      .connect(user2)
      .approve(stargateBorrow.address, ethers.constants.MaxUint256);
    ...
    await lendingPool
      .connect(user2)
      .deposit(wethAddress, wethAmt, user2.address, 0);
