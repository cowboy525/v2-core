...
describe("MultiFeeDistribution", () => {
  ...
  beforeEach(async () => {
    ...
    const mockChefFactory = await ethers.getContractFactory(
      "MockIncentivesController"
    );
    const mockChef = await mockChefFactory.deploy();
    await mockChef.deployed();

    const mockMiddleFactory = await ethers.getContractFactory(
      "MockMiddleFeeDistribution"
    );
    ...
    await mfd.setLockTypeInfo(
      HardhatDeployConfig.LOCK_INFO.LOCK_PERIOD,
      HardhatDeployConfig.LOCK_INFO.MULTIPLIER
    );

    await radiant
      .connect(user1)
      .approve(mfd.address, ethers.constants.MaxUint256);
    await radiant
      .connect(user2)
      .approve(mfd.address, ethers.constants.MaxUint256);
  });
  ...
  it("setDefaultRelockTypeIndex", async () => {
    await expect(
      mfd.connect(user1).setDefaultRelockTypeIndex(4)
    ).to.be.reverted;
  ...
  it("setLockTypeInfo", async () => {
    ...
    await expect(
      mfd
        .connect(user1)
        .stake(ethers.utils.parseUnits("1", 18), user1.address, 4)
    ).to.be.reverted;
  });
  ...
  it("mint & stake vlidation", async () => {
    ...
    await expect(mfd.connect(user1).mint(user1.address, depositAmount, true)).to
      .be.reverted;
  ...
  it("Different lock periods", async () => {
    ...
    expect((await mfd.lockedBalances(user1.address)).unlockable).to.be.equal(
      depositAmount
    );
  ...
  it("Linear exit; day 1", async () => {
    ...
    const penaltyFactor = Math.floor(
      QUART + (HALF * (unlockTime - blockTimestamp)) / MFD_VEST_DURATION
    );
  ...
  it("Individual early exit; validation", async () => {
    ...
    await expect(
      mfd.connect(user1).individualEarlyExit(true, timestamp - 1)
    ).to.be.reverted;
  });
  ...
  it("Individual early exit; with penalty", async () => {
    ...
    await mfd
      .connect(user1)
      .individualEarlyExit(true, timestamp + MFD_VEST_DURATION);
  ...
  it("getReward; unknown token", async () => {
    await expect(
      mfd.connect(user1).getReward([user1.address])
    ).to.be.reverted;
  });
