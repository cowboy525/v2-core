...
import { LendingPool } from "../../typechain-types/protocol/lendingpool";
...
describe("Individual Early Exits", () => {
  ...
  before(async () => {
    ...
    BURN = deployConfig.MFD_BURN_RATIO;
  ...
  it("Check Individual Early Exit.", async () => {
    ...
    await lendingPool
      .connect(user1)
      .deposit(wethAddress, depositAmtWeth, user1.address, 0);

    // Become Emission Eligible
    await zapIntoEligibility(user1, deployData);

    const VEST_DURATION = await lpMfd.VEST_DURATION();
    ...
    const penaltyFactor = Math.floor(
      QUART +
      (HALF * (unlockTime - blockTimestamp - 1000)) / VEST_DURATION.toNumber()
    );
    ...
    await mfd
      .connect(user1)
      .individualEarlyExit(false, unlockTime);
    const userBal1 = await radiant.balanceOf(user1.address);
    const daoBal1 = await radiant.balanceOf(dao.address);
