...
describe("Require Locked Value", () => {
  ...
  it("Lock new tokens, locked eth values are correct", async () => {
    ...
    const expectedLockedUsdVal = lpTokenPriceUsd
      .mul(lpBalance)
  ...
  it("Unlock all, earnings go to zero; but prev emissions are kept", async () => {
    const rewards0 = await chef.pendingRewards(
      user2.address,
      deployData.allTokenAddrs
    );
    ...
    const rewards1 = await chef.pendingRewards(
      user2.address,
      deployData.allTokenAddrs
    );
    ...
    const totalRewards0 = userBaseClaimable0.add(
      rewards0.reduce((a, b) => a.add(b))
    );
