...
describe("Looping/Leverager", () => {
  ...
  it("borrows & deposits correct amount, w/ correct fee", async () => {
    ...
    await wethGateway
      .connect(user2)
      .depositETH(lendingPool.address, user2.address, 0, {
    ...
    await leverager
      .connect(user2)
      .loop(usdcAddress, amt, 2, borrowRatio, loops, false);

    const initialFee = amt.mul(FEE_LOOPING).div(1e4);
    const initialDeposit = amt.sub(initialFee);

    const loop1Borrow = initialDeposit
      .mul(BigNumber.from(borrowRatio))
      .div(1e4);
  ...
  it("autoZap while looping", async () => {
    ...
    await lendingPool
      .connect(user2)
      .deposit(usdcAddress, testUSDCDeposit, user2.address, 0);

    await lendingPool
      .connect(user2)
      .borrow(usdcAddress, testUSDCBorrow, 2, 0, user2.address);
    ...
    await vdWETH
      .connect(user2)
      .approveDelegation(wethGateway.address, ethers.constants.MaxUint256);

    await wethGateway
      .connect(user3)
      .depositETH(lendingPool.address, user3.address, 0, {
        value: ethers.utils.parseEther("5000"),
      });

    await wethGateway
      .connect(user2)
      .borrowETH(lendingPool.address, ethers.utils.parseEther("0.04"), 2, 0);
