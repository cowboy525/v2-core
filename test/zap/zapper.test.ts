...
describe("Zapper", function () {
  ...
  it("setPoolHelper", async function () {
    await expect(
      lockZap.connect(user2).setPoolHelper(poolHelperAddress)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  ...
  it("can zap into locked lp", async function () {
    ...
    const LP = <MockToken>(
      await ethers.getContractAt("MockToken", deployData.stakingToken)
    );
  ...
  it("can zap from Vesting", async function () {
    ...
    await lendingPool
      .connect(user2)
      .deposit(usdcAddress, depositAmt, user2.address, 0);
    ...
    const wethRequired = await lockZap
      .connect(user2)
      .quoteFromToken(totalVesting);
  ...
  it("can zap WETH, and from Borrow", async function () {
    ...
    await lendingPool
      .connect(user3)
      .deposit(wethAddress, depositAmtWeth.mul(5), user3.address, 0);

    expect(
      (await lendingPool.getUserAccountData(user3.address)).totalCollateralETH
    ...
    expect(
      (await lendingPool.getUserAccountData(user3.address)).totalDebtETH
    ).to.be.gt(BigNumber.from(0));
  });

  it("can zap from Vesting w/ Borrow", async function () {
    // Become eligilble for rewards;
    await lockZap.connect(user4).zap(
      false,
      0,
      0,
      0,
    ...
    await WETH.connect(user4).approve(
      lockZap.address,
      ethers.constants.MaxUint256
    );
    ...
    await lendingPool
      .connect(user4)
      .deposit(wethAddress, depositAmtWeth.mul(5), user4.address, 0);
    ...
    expect(
      (await lendingPool.getUserAccountData(user4.address)).totalDebtETH
    ...
    await lendingPool
      .connect(user4)
      .borrow(wethAddress, depositAmtWeth.mul(4), 2, 0, user4.address);
  ...
  it("can early exit after zapping vesting w/ borrow", async function () {
    ...
    await lendingPool
      .connect(user4)
      .deposit(wethAddress, depositAmtWeth, user4.address, 0);

    expect(
      (await lendingPool.getUserAccountData(user4.address)).totalCollateralETH
    ).to.be.gt(BigNumber.from(0));
    expect(
      (await lendingPool.getUserAccountData(user4.address)).totalDebtETH
    ).to.equal(BigNumber.from(0));
    ...
    await chefIncentivesController.claim(user4.address, [rWETHAddress]);
    expect((await mfd.earnedBalances(user4.address)).total).to.be.gt(
      BigNumber.from(0)
    );
