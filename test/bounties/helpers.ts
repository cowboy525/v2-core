...
import { VariableDebtToken, LendingPool, AToken, WETH, MockToken } from "../../typechain-types";
import { getLatestBlockTimestamp } from "../shared/helpers";

export const zap = async (user: SignerWithAddress, deployData: DeployData, borrow: boolean, defaultLockTime: number) => {
    ...
    if (borrow) {
        await (await lockZap.connect(user).zapOnBehalf(
            borrow,
...
export const deposit = async (ticker: string, amount: string, user: SignerWithAddress, lendingPool: LendingPool, deployData: DeployData) => {
    if (amount === "0") return;
...
export const doBorrow = async (ticker: string, amount: string, user: SignerWithAddress, lendingPool: LendingPool, deployData: DeployData) => {
    const token = <AToken>await ethers.getContractAt("AToken", deployData.allTokens[ticker]);
    const underlying = await token.UNDERLYING_ASSET_ADDRESS();
    let asset;
    let amt;
