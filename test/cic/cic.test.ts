...
describe("Non-Elig CIC", () => {
  ...
  before(async () => {
    ...
    const onwardIncentiveControllerFactory = await ethers.getContractFactory(
    ...
    rUSDC = <AToken>(
      await ethers.getContractAt("AToken", rUSDCAddress)
    );

    await chefIncentivesController.setEligibilityEnabled(false);
    await chefIncentivesController.setOnwardIncentives(
      rUSDCAddress,
      onwardIncentiveController.address
    );
  ...
  describe("addPool requires", () => {
    it("should be callable by only pool configurator", async () => {
      ...
      await expect(chef.connect(user1).addPool(user1.address, 10)).to.be
        .reverted;
  ...
  describe("handleActionAfter", () => {
    it("can't called from non-pool", async () => {
      await expect(
        chefIncentivesController.handleActionAfter(user1.address, 10, 10)
      ).to.be.reverted;
    });
  });
  ...
  describe("batchUpdateAllocPoint", () => {
    it("length must match", async () => {
      await expect(
        chefIncentivesController.batchUpdateAllocPoint([user1.address], [])
      ).to.be.reverted;
    });

    it("non existing pool", async () => {
      await expect(
        chefIncentivesController.batchUpdateAllocPoint([user1.address], [10])
      ).to.be.reverted;
    });
  });
  ...
  describe("setRewardsPerSecond callable", () => {
    it("should be callable by only owner", async () => {
      await expect(
        chefIncentivesController
          .connect(user1)
          .setRewardsPerSecond(rewardsPerSecond, true)
      ).to.be.revertedWith("Ownable: caller is not the owner");

      await expect(
        chefIncentivesController
          .connect(deployer)
          .setRewardsPerSecond(rewardsPerSecond, false)
      ).to.be.not.reverted;
  ...
  describe("check persist rewards per second in a single pool", () => {
    ...
    it("Deposit by User 1", async () => {
      ...
      await lendingPool
        .connect(user1)
        .deposit(usdcAddress, depositAmt.div(2), user1.address, 0);
      await lendingPool
        .connect(user1)
        .deposit(usdcAddress, depositAmt.div(2), user1.address, 0);
      ...
      const userInfo = await chefIncentivesController.userInfo(
        rUSDCAddress,
        user1.address
      );
    ...
    it("claimable rewards should be calculated in seconds", async () => {
      const claimableRewards0 = await chefIncentivesController.pendingRewards(
        user1.address,
        [rUSDCAddress]
      );
      ...
      const claimableRewards1 = await chefIncentivesController.pendingRewards(
        user1.address,
        [rUSDCAddress]
      );
      const allReward = await chefIncentivesController.allPendingRewards(
        user1.address
      );
      ...
      const expectedRewardsNum = parseFloat(
    ...
    it("claimable rewards should be updated with rewards per seconds", async () => {
      ...
      await chefIncentivesController
        .connect(deployer)
        .setRewardsPerSecond(rewardsPerSecond2, true);

      const claimableRewards2 = await chefIncentivesController.pendingRewards(
        user1.address,
        [rUSDCAddress]
      );
      ...
      const claimableRewards3 = await chefIncentivesController.pendingRewards(
        user1.address,
        [rUSDCAddress]
      );

      // new rewards should be increased 3 times, but not by mined period
      const prevRewards = claimableRewards1[0]
        .sub(claimableRewards0[0])
        .mul(times);
      const newRewards = claimableRewards3[0].sub(claimableRewards2[0]);
      expect(
        parseFloat(ethers.utils.formatEther(newRewards))
      ).to.be.approximately(
    ...
    it("should claim rewards", async () => {
       ...
       * will be different.
       **/
      await chefIncentivesController.setRewardsPerSecond(
        ethers.utils.parseUnits("1", 34),
        true
      );
      await advanceTimeAndBlock(500);
      await expect(
        chefIncentivesController.connect(user1).claim(user1.address, [user1.address])
      ).to.be.reverted;
  ...
  describe("ChefIncentivesController Rewards Schedule and Manual Setting RPS.", () => {
    ...
    it("scheulded rewards", async () => {
      ...
      const startTimeOffset =
        (await getLatestBlockTimestamp()) - startTime.toNumber();

      const cicStartTimeOffSets = [
        startTimeOffset + 100,
        startTimeOffset + 500,
      ...
      await expect(
        chefIncentivesController.connect(deployer).setEmissionSchedule([], [])
      ).to.be.reverted;

      await expect(
        chefIncentivesController
          .connect(deployer)
          .setEmissionSchedule([0], [100])
      ).to.be.reverted;
  ...
  it("registerRewardDeposit", async () => {
    ...
    expect(await chefIncentivesController.rewardsPerSecond()).to.be.equal(
      await chefIncentivesController.lastRPS()
    );
  });
  ...
  it("recover ERC20", async () => {
    ...
    expect(
      await mockErc20.balanceOf(chefIncentivesController.address)
