...
describe("Ensure Users can borrow after Eligibility change (no sub revert)", () => {
  ...
  it("Deposit and Lock by User 2", async () => {
    ...
    await lendingPool
      .connect(user2)
      .deposit(usdcAddress, "1000000", user2.address, 0);
  ...
  it("Wait for lock expire, user 2 borrow, ensure no revert", async () => {
    ...
    await lendingPool
      .connect(user2)
      .borrow(usdcAddress, "100000", 2, 0, user2.address);

    await expect(chef.pendingRewards(user2.address, deployData.allTokenAddrs))
      .to.be.not.reverted;
  });
});
