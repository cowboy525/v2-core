...
describe('CIC Basic functionalties', () => {
	...
	describe("owner permission", async () => {
		it('setBountyManager', async () => {
			await expect(cic.connect(user1).setBountyManager(deployer.address)).to.be.revertedWith("Ownable: caller is not the owner");
		});

		it('setEligibilityEnabled', async () => {
			await expect(cic.connect(user1).setEligibilityEnabled(true)).to.be.revertedWith("Ownable: caller is not the owner");
		});
		...
		it('batchUpdateAllocPoint', async () => {
			await expect(cic.connect(user1).batchUpdateAllocPoint([], [])).to.be.revertedWith("Ownable: caller is not the owner");
		});

		it('setEmissionSchedule', async () => {
			await expect(cic.connect(user1).setEmissionSchedule([], [])).to.be.revertedWith("Ownable: caller is not the owner");
		});

		it('setLeverager', async () => {
			await expect(cic.connect(user1).setLeverager(deployer.address)).to.be.revertedWith("Ownable: caller is not the owner");
		});

		it('setEligibilityExempt', async () => {
			await expect(cic.connect(user1).setEligibilityExempt(deployer.address, true)).to.be.revertedWith("InsufficientPermission");
		});

		it('setEndingTimeUpdateCadence', async () => {
			await expect(cic.connect(user1).setEndingTimeUpdateCadence(10)).to.be.revertedWith("Ownable: caller is not the owner");
		});

		it('registerRewardDeposit', async () => {
			await expect(cic.connect(user1).registerRewardDeposit(10)).to.be.revertedWith("Ownable: caller is not the owner");
		});
		...
		it('recoverERC20', async () => {
			await expect(cic.connect(user1).recoverERC20(deployer.address, 10)).to.be.revertedWith("Ownable: caller is not the owner");
		});
	});
