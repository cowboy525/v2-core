...
contract AutoCompounder is Ownable {
	...
	constructor(address _uniRouter, address _mfd, address _baseToken, address _addressProvider, address _lockZap) {
		require(_uniRouter != address(0));
		require(_mfd != address(0));
		require(_baseToken != address(0));
		require(_addressProvider != address(0));
		...
		priceProvider = IMultiFeeDistribution(multiFeeDistribution).PriceProvider();
	...
	function claimCompound(address _user, bool _execute) public returns (uint256 fee) {
		...
		(address[] memory tokens, uint256[] memory amts) = _viewPendingRewards(_user);
		...
		if (!_execute) {
			...
			uint256 compoundFee = (pendingInRdnt * FEE) / PERCENT_DIVISOR;
			return compoundFee;
		}

		uint256 actualWethAfterSwap = _claimAndSwapToBase(_user);
		require((PERCENT_DIVISOR * actualWethAfterSwap) / noSlippagePendingEth >= 9500, "too much slippage");

		if (isAutoCompound) {
			fee = _wethToRdnt(((actualWethAfterSwap * FEE) / PERCENT_DIVISOR), _execute);
		}
	...
	function _viewPendingRewards(address _user) internal view returns (address[] memory tokens, uint256[] memory amts) {
	...
	function _quoteSwapWithOracles(
		...
		require(_in.length == _amtsIn.length);
	...
	function _wethToRdnt(uint256 _wethIn, bool _execute) internal returns (uint256 rdntOut) {
		...
		require(ethValueOfRDNT / 10 ** 8 >= (_wethIn * 9500) / 10000, "too much slippage");
	}
