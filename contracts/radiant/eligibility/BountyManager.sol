...
contract BountyManager is Initializable, OwnableUpgradeable, PausableUpgradeable, RecoverERC20 {
	 ...
	 * _getBestBounty returns this based on priority (expired locks first, then inelig emissions, then autocompound)
	 */
	...
	function executeBounty(
		...
		(incentivizer, totalBounty, issueBaseBounty, actionType) = _getBestBounty(_user, _execute, _actionType);
		if (issueBaseBounty) {
