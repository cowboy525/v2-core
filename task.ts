...
task('updateAllocPoints', 'Updates allocation points')
	...
	.setAction(async (taskArgs, hre) => {
		const ChefIncentivesControllerFactory = await hre.ethers.getContractFactory('ChefIncentivesController');
		const chefIncentivesControllerContract = ChefIncentivesControllerFactory.attach(
			taskArgs.chefIncentivesControllerAddress
		);
		const receipt = await chefIncentivesControllerContract.batchUpdateAllocPoint(
...
task('deployNewAsset', 'Deploy A token, Debt Tokens, Risk Parameters and configure asset')
	...
	.addParam('lendingPoolAddressesProviderAddress', 'LendingPoolAddressesProvider Address')
	.addParam('chefIncentivesControllerAddress', 'ChefIncentivesController Address')
	...
	.setAction(
		async (
			...
			const lendingPoolAddressesProvider = LendingPoolAddressesProviderFactory.attach(
				lendingPoolAddressesProviderAddress
			);
			...
			const LendingPoolConfiguratorFactory = await hre.ethers.getContractFactory('LendingPoolConfigurator');
			const lendingPoolConfigurator = LendingPoolConfiguratorFactory.attach(lendingPoolConfiguratorProxyAddress);
			...
			await lendingPoolConfigurator.setReserveFactor(configuration.asset, configuration.reserveFactor);

			const AaveOracleFactory = await hre.ethers.getContractFactory('AaveOracle');
			const aaveOracle = AaveOracleFactory.attach(await lendingPoolAddressesProvider.getPriceOracle());
			await aaveOracle.setAssetSources([reserveAssetAddress], [chainlinkAggregator]);
...
task('getAllReservesData', 'Gets all reserves data')
	...
	.setAction(async ({aaveProtocolDataProviderAddress}, hre) => {
		const AaveProtocolDataProviderAddress = await hre.ethers.getContractFactory('AaveProtocolDataProvider');
		const aaveProtocolDataProviderFactory = AaveProtocolDataProviderAddress.attach(aaveProtocolDataProviderAddress);

		const addrs = await aaveProtocolDataProviderFactory.getReserveTokensAddresses(
			'0x774382EF196781400a335AF0c4219eEd684ED713'
		);
		console.log(addrs);

		const allReservesTokens = await aaveProtocolDataProviderFactory.getAllReservesTokens();

		const allReservesData = await Promise.all(
			allReservesTokens.map((entry: any) => aaveProtocolDataProviderFactory.getReserveData(entry.tokenAddress))
		);
...
task('cleanLocks', 'Clean expired locks').setAction(async (taskArgs, hre) => {
	...
	const config = JSON.parse(fs.readFileSync(`./export/${chainId}-frontend.json`).toString());

	const CIC = await hre.ethers.getContractAt('ChefIncentivesController', config.chefIncentivesController);

	const middleFeeDistribution = await hre.ethers.getContractAt('MiddleFeeDistribution', await CIC.rewardMinter());

	const mfdAddress = await middleFeeDistribution.multiFeeDistribution();

	const MFD = await hre.ethers.getContractAt('MultiFeeDistribution', mfdAddress);

	const limit = 50;

	console.log('MultiFeeDistribution...');
	const MFDlockerlist = await hre.ethers.getContractAt('LockerList', await MFD.userlist());
...
task('savePendingRewards', 'Save pending rewards').setAction(async (taskArgs, hre) => {
	const {chainId} = await hre.ethers.provider.getNetwork();
	console.log('Chain Id:', chainId);
	const config: DeployData = JSON.parse(fs.readFileSync(`./export/${chainId}-frontend.json`).toString());

	const CIC = await hre.ethers.getContractAt('ChefIncentivesController', config.chefIncentivesController);

	const MFD = await hre.ethers.getContractAt('MultiFeeDistribution', config.multiFeeDistribution);
	...
	lockers = lockers.filter(
		(user, index) => lockers.indexOf(user) === index && user !== hre.ethers.constants.AddressZero
	);
...
task('withdrawLP', 'Withdraw LP from MFD')
	...
	.setAction(async (taskArgs, hre) => {
		...
		if (withdrawable.gt(0)) {
			const receipt = await MFD.recoverERC20(await MFD.stakingToken(), withdrawable);
			await receipt.wait();
		}
