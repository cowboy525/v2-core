...
(async () => {
	...
	for (let i = 0; i < chainIds.length; i += 1) {
		const config = JSON.parse(
			fs.readFileSync(`./deployments/${directories[chainIds[i]]}/RadiantOFT.json`).toString()
		);
	...
	for (const [curChain, oftAddress] of Object.entries(ofts)) {
		if (curChain != chainId.toString()) {
			let trustedRemote = ethers.utils.solidityPack(['address', 'address'], [oftAddress, rdnt.address]);

			console.log(
				`setTrustedRemote from ${chainId} to ${curChain} with ${lzChainId[curChain]} and ${oftAddress}`
			);
			console.log(lzChainId[curChain]);
